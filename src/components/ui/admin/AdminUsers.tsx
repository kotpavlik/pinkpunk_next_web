...
const AdminUsers = () => {
    ...
    const filteredUsers = useMemo(() => {
        ...
        return list
    }, [users, searchTerm, onlyWithPhone, onlyWithOrders, ordersSort])
        ...
        <div className="p-4 mt-4 bg-white/5 backdrop-blur-md border border-white/10 text-white">
            ...
            <div className="mb-4 space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-3 text-sm">
                    ...
                    {(onlyWithPhone || onlyWithOrders || ordersSort !== 'none') && (
                        <button
                            type="button"
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-5">
                <div className="bg-white/10 backdrop-blur-sm border border-white/20 p-3">
                    <div className="text-xs text-white/60 mb-1">Клиентов</div>
                ...
                <div className="bg-white/10 backdrop-blur-sm border border-white/20 p-3">
                    ...
                    <div className="text-2xl font-bold text-white">{users.filter(u => phoneFromRow(u)).length}</div>
                </div>
                <div className="bg-white/10 backdrop-blur-sm border border-white/20 p-3">
                    <div className="text-xs text-white/60 mb-1">Всего заказов (все статусы)</div>
                    <div className="text-2xl font-bold text-[var(--mint-bright)]">{totalOrdersAll}</div>
                </div>
                <div className="bg-white/10 backdrop-blur-sm border border-white/20 p-3">
                    <div className="text-xs text-white/60 mb-1">Сумма (оплачено + завершено)</div>
            ...
            <div>
                {status === 'loading' ? (
                    ...
                    <div className="text-center py-10 text-white/60">
                        {searchTerm || onlyWithPhone || onlyWithOrders ? 'Никто не подошёл под фильтры' : 'Список пуст'}
                    </div>
                ) : (
